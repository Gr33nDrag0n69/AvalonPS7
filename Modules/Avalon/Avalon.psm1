...
# This function parses voltage/current/power status strings from Avalon miners
# IT IS NOT RELIABLE: different models and firmware versions report different formats
# Will required more documentation and test
# Q      : PS[0 1216 2364 54 1296 2365 1409]
# Nano3S : PS[0 0 27416 4 0 3756 131]

function Convert-AvalonVoltageString {
    ...
    # Extract integers
    $nums = [regex]::Matches($VoltageString, '-?\d+') | ForEach-Object { [int]$_.Value }

    if ($nums.Count -eq 0) {
        Write-Error 'No numbers found in input.'
        return
    }

    # Helper to format voltage entries encoded as value/100
    $fmtV = { param($v) ('{0:N2} V' -f ($v / 100.0)) }
    $fmtA = { param($a) ('{0} A' -f $a) }
    $fmtW = { param($w) ('{0} W' -f $w) }
    ...
    switch ($nums.Count) {
        ...
        6 {
            ...
            $Output = [PSCustomObject]@{
                ...
                'HashBoardsVoltage'    = $hashV
                'HashBoardsCurrent'    = $amps
                'HashBoardsPower'      = $watts
                'HashBoardsSetVoltage' = $setV
                'Note'                 = '6-field format (standard). Voltages are value/100 per Avalon docs.'
            }
        }
        ...
        default {
            ...
            $Output = [PSCustomObject]@{
                'RawNumbers' = $nums
                'Note'       = "Unexpected number of fields ($($nums.Count)). Check ascset help or miner model doc."
            }
        }
    }
